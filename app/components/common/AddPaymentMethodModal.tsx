  ...
  return (
    <View style={styles.formContainer}>
      ...
      <Text style={styles.platformWarning}>
        Stripe card input is only available in web mode. Please use the web
        version to add a payment method.
      </Text>
...
const AddPaymentMethodModal: React.FC<AddPaymentMethodModalProps> = ({
  ...
  const handleSaveCard = async (stripe: any, elements: any) => {
    ...
    try {
      ...
      if (attachError) {
        throw new Error(
          attachError.message || "Failed to attach payment method."
        );
      }

      if (attachData?.error) {
        throw new Error(
          attachData.error || "Server error attaching payment method."
        );
      }
