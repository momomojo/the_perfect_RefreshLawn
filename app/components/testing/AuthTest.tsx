...
export default function AuthTest() {
  ...
  const authOperations: AuthOperation[] = [
    ...
    {
      name: "getUserRole",
      description: "Get role from JWT claims",
      requiredFields: [],
      action: async () => {
        ...
        if (!sessionData.session) {
          return { error: "No active session found. Please sign in first." };
        }

        // Get the raw JWT to inspect
        const accessToken = sessionData.session?.access_token;

        // Decode JWT manually to check claims (without verification)
        ...
        // Get user role from various possible locations
        const appMetadata = sessionData.session?.user?.app_metadata;

        // Check for user_role in multiple locations (in order of priority):
        // 1. JWT's root level user_role claim (set by custom hook)
        // 2. JWT's app_metadata.role (also set by custom hook)
        // 3. JWT's user_metadata.role (sometimes set during signup)
        const userRole =
          decodedToken?.user_role ||
          appMetadata?.role ||
          sessionData.session?.user?.user_metadata?.role;

        // Fetch user profile directly from the database as a fallback
        let databaseUserRole = null;
        if (sessionData.session?.user?.id) {
          try {
            ...
            if (error) {
              ...
              databaseUserRole = profileData.role;
            }
          } catch (error) {
        ...
        return {
          ...
          tokenMatches:
            userRole === databaseUserRole
              ? "✅ JWT matches database"
              : "❌ JWT does not match database",
          jwtHookEnabled: userRole
            ? "✅ JWT hook is working"
            : "❌ JWT hook may not be enabled",
        };
      },
    },
  ];
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Add JWT Debugger for more detailed diagnostics */}
      {user && (
