      ...
      .then(({ paymentIntent }) => {
        ...
        switch (paymentIntent?.status) {
          ...
          case "processing":
            ...
            setMessage(
              "Your payment is processing. We will update you when it is complete."
            );
...
const PaymentCompleteScreen: React.FC = () => {
  ...
  // Native platforms don't use this redirect flow - payment completes in PaymentSheet
  useEffect(() => {
    if (Platform.OS !== "web") {
      console.log(
        "[PaymentComplete] Native platform detected, redirecting to dashboard"
      );
      router.replace("/(customer)/dashboard");
    }
  }, []);

  // Only render for web platform
  if (Platform.OS !== "web") {
    return (
      <SafeAreaView
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F9FAFB" }}>
      <Stack.Screen
        options={{ title: "Payment Status", headerLeft: () => null }}
      />
