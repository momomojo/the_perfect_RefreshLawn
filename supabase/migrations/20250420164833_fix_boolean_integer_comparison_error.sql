-- Fix the update_booking_status function to properly handle the boolean > integer comparison error
...
DECLARE
  ...
  v_success INTEGER; -- Changed from BOOLEAN to INTEGER
BEGIN
  ...
  -- Changed to handle integer comparison correctly
  RETURN v_success > 0;
END;
$function$;

COMMENT ON FUNCTION public.update_booking_status(uuid, public.booking_status, uuid) IS 'Updates booking status with role-based permissions, fixed INTEGER v_success variable type'; 