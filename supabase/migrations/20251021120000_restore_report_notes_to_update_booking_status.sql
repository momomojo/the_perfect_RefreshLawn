-- Migration: Restore report_notes parameter to update_booking_status function
-- Description: The function was overwritten without the p_report_notes parameter.
--              This migration restores it with proper functionality.

-- Drop the existing 3-parameter version
DROP FUNCTION IF EXISTS public.update_booking_status(uuid, public.booking_status, uuid);

-- Create the corrected 4-parameter version
...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
    ...
    ELSIF v_user_role = 'technician' THEN
      ...
      IF p_new_status NOT IN ('in_progress', 'completed', 'cancelled', 'rescheduled') THEN
