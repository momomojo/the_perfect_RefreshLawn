-- Migration: Fix service_date reference in booking notification
-- Description: Replace references to NEW.service_date with NEW.scheduled_date in create_booking_status_notification

-- Replace the function definition with updated date field
...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    -- Only create notifications for user-actionable status changes
    CASE NEW.status
      ...
      WHEN 'scheduled' THEN
        ...
        -- Use scheduled_date instead of non-existent service_date
