-- Migration: Fix incomplete jsonb_build_object call in update_booking_status function
-- Description: This fixes a syntax error in the update_booking_status function where the jsonb_build_object call for technician notification was incomplete
...
BEGIN
  ...
  BEGIN
    ...
    IF v_customer_id IS NOT NULL THEN
      PERFORM public.create_notification(v_customer_id, 'booking_updated', v_notification_title, v_notification_message, jsonb_build_object('booking_id', p_booking_id, 'new_status', p_new_status));
    END IF;
    IF v_technician_id IS NOT NULL AND p_new_status IN ('in_progress','completed','cancelled','rescheduled') THEN
      PERFORM public.create_notification(v_technician_id, 'booking_updated', v_notification_title, 'The status of your assigned booking (ID: '||left(p_booking_id::text,8)||') is now '||p_new_status||'.', jsonb_build_object('booking_id', p_booking_id, 'new_status', p_new_status));
    END IF;
...
COMMENT ON FUNCTION public.update_booking_status(uuid, public.booking_status, uuid) IS 'Updates booking status with role-based permissions, using enum type'; 